...
def deepseek_generate_story(topic: str, page_count: int, outline: str = "", character_desc: str = "") -> dict:
    ...
    prompt = f"""你是专业儿童绘本作者，严格遵循儿童内容创作规范。请根据我提供的主题和故事梗概，创作一个 {page_count} 页的儿童绘本故事。
...
【严格遵守4大约束（论文要求）】
1. 故事结构：开头-发展-结尾完整，逻辑清晰
2. 语言难度：简单短句、用词浅显、每頁1-2句
3. 内容安全：正向、温暖、无危险、无暴力、无恐怖内容
4. 教育性：传递友善、勇敢、分享、好奇等正向价值
...
hand-drawn texture, nostalgic storybook charm, no mixed features, no strange body,
high quality,correct anatomy, no deformation,single main character, ONLY ONE ANIMAL, 4K, {text}
"""
...
def get_recommend_books(userId: str = None, db: sqlite3.Connection = Depends(get_db)):
    ...
    c.execute(f"""
        ...
        ORDER BY click_count DESC, create_time DESC
        LIMIT 3
    """)
    ...
    if top_ids:
        ...
        if latest_rows:
            latest_book = dict(latest_rows[0])
    else:
        c.execute(f"""
            SELECT book_id, title, page_count, create_time, user_id, click_count, is_vip_only
            FROM book {vip_filter}
    ...
    # 3. 添加封面和作者信息
    all_books = top_books + ([latest_book] if latest_book else [])
